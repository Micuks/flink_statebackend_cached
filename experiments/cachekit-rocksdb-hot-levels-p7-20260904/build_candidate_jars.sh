...
./mvnw -pl flink-state-backends/flink-statebackend-rocksdb -am \
  "${common_maven[@]}" \
  -Dtest=RocksDBStateBackendConfigTest,RocksDBResourceContainerTest \
...
overlay=$(mktemp -d /tmp/cachekit-p7-overlay.XXXXXX)
cleanup() {
  [[ $overlay == /tmp/cachekit-p7-overlay.* && -d $overlay ]] && rm -rf -- "$overlay"
}
trap cleanup EXIT
...
build_one() {
  local arch=$1 expected_base_sha=$2 base candidate base_sha
  base=$target_dir/flink-statebackend-cachekit-1.16-SNAPSHOT-p4-$arch.jar
  candidate=$target_dir/flink-statebackend-cachekit-1.16-SNAPSHOT-p7-$arch.jar

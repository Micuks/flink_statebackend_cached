...
expected_source=0a705bc708332f7ef0e63f017eb2c2dd4ce4092e
expected_x86_p16_sha=f4bc662188d975cde8757ad77c313cb5e3dbf634ddc287e2d2ac4a9cca5c31d2
expected_arm_p16_sha=16b4b8e83b8693671a8a3f0fed6d48dafb0f24b86921ee4ad46fbf31be30288a
...
overlay=$(mktemp -d /tmp/cachekit-p17-overlay.XXXXXX)
cleanup() {
  [[ $overlay == /tmp/cachekit-p17-overlay.* && -d $overlay ]] && rm -rf -- "$overlay"
}
trap cleanup EXIT
...
build_one() {
  ...
  base=$target_dir/flink-statebackend-cachekit-1.16-SNAPSHOT-p16-$arch.jar
  candidate=$target_dir/flink-statebackend-cachekit-1.16-SNAPSHOT-p17-$arch.jar
  [[ -f $base ]] || { echo "missing P16 base: $base" >&2; exit 68; }
  base_sha=$(sha256sum "$base" | awk '{print $1}')
  [[ $base_sha == "$expected_base_sha" ]] || {
    echo "unexpected $arch P16 base: $base_sha" >&2
    exit 69
  }
  cp "$base" "$candidate"
  ...
  comm -23 "$overlay/$arch.base.entries" "$overlay/$arch.candidate.entries" \
    >"$overlay/$arch.removed.entries"
  comm -13 "$overlay/$arch.base.entries" "$overlay/$arch.candidate.entries" \
    >"$overlay/$arch.added.entries"
  [[ ! -s $overlay/$arch.removed.entries ]] || {
    echo "$arch candidate removed entries" >&2
    exit 70
  }
  diff -u <(printf '%s\n' \
      'org/apache/flink/contrib/streaming/state/cachekit/state/CachedInternalMapState$SingletonSnapshotIterable.class' \
      'org/apache/flink/contrib/streaming/state/cachekit/state/CachedInternalMapState$SingletonSnapshotIteratorEntry.class') \
    "$overlay/$arch.added.entries" || {
      echo "$arch candidate has unexpected added entries" >&2
      exit 70
    }
...
echo "schema=cachekit-p17-artifact-overlay-audit-v1"
echo "source_commit=$expected_source"
echo "focused_tests=43"
echo "overlay_stem=$stem"
echo 'expected_new_entries=SingletonSnapshotIterable,SingletonSnapshotIteratorEntry'
echo "entry_set_rule=no removals and exactly two expected entries added"
echo "non_overlay_entries_identical_to_p16=true"
build_one x86 "$expected_x86_p16_sha"
build_one aarch64 "$expected_arm_p16_sha"

#!/usr/bin/env python3
"""Summarize the P4 q9 paired screen and its activation/performance gates."""
...
def main() -> None:
    ...
    parser.add_argument("--output-prefix", default="P4_RESULT")
    ...
    for result_path in sorted((args.expdir / "results" / "raw").glob("*/result.json")):
        ...
        activation_path = leg / "COMPRESSION_AUDIT.json"
        activation = (
    ...
    control_rows = [row for row in rows if row["variant"] == args.control]
    activation_pass = bool(candidate_rows) and bool(control_rows) and all(
    ...
    result = {
        "schema": "cachekit-rocksdb-no-compression-p4-summary-v1",
    ...
    for row in rows:
        ...
        observed = activation.get("observed_counts", {})
        applied = ", ".join(f"{mode}={count}" for mode, count in observed.items())
        lines.append(
            f"| {row['query']} | {row['round']} | {row['variant']} | "
            f"{row['kps_core']:.2f} | {row['raw_kps']:.2f} | {row['cores']:.2f} | "
            f"{'yes' if row['valid'] else 'no'} | {applied or 'missing'} |"
        )
    lines.extend(
        [
            "",
    ...
    markdown = "\n".join(lines) + "\n"
    for path in (final / f"{args.output_prefix}_{stamp}.json", final / f"{args.output_prefix}.json"):
        path.write_text(payload)
    for path in (final / f"{args.output_prefix}_{stamp}.md", final / f"{args.output_prefix}.md"):

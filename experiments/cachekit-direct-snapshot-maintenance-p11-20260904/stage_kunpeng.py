...
TARGET_EXPERIMENT = (
    "/home/wuql/flink-cluster/experiments/"
    "cachekit-p11-direct-snapshot-maintenance-q9-100m-kunpeng-numa0-20260904"
)
TARGET_PROJECT = "ckkp5a9p11n0"
SOURCE_SCRATCH = "/tmp/ckkp5a9p10n0"
TARGET_SCRATCH = "/tmp/ckkp5a9p11n0"
SOURCE_PORTS = ("10850", "11949", "11950")
TARGET_PORTS = ("10858", "11965", "11966")
SOURCE_COMMIT = "29d60d23dfaf27315df7608a495e0b35de237112"
ARTIFACT_SHA256 = (
    "de935deec5eb709c8afbae79e1161a9ecaadc30c5e7cc89602b04404ca2c3e29"
)
TARGET_CPUSET = (
    "38,40,42,44,46,48,50,52,54,56,58,60,62,64,66,68,70,72,74"
)
TARGET_CPUSET_MEMS = "0"
REMOTE_STAGE = "/tmp/cachekit-p11-direct-snapshot-kunpeng-stage-20260904"
...
if not (source_exp / "final/P10_SCREEN_SUMMARY.json").is_file():
    raise SystemExit("source P10 screen summary is incomplete")
...
for variant in variants:
    ...
    shutil.copy2(
        source_exp / "variants/hot2-a/flink-conf.yaml",
...
shutil.copy2(
    stage / "flink-statebackend-cachekit-1.16-SNAPSHOT-p11-aarch64.jar",
...
for old, new in (
    (
        ...
        "variants=(hot2-a hot2-direct-maintained hot2-direct-maintained-64k)",
    ),
    (
        "expected_hot={'baseline':'0','hot2-a':'2','hot2-overlay':'2','hot2-maintained':'2','hot2-overlay-64k':'2','hot2-maintained-64k':'2'}[variant]",
        "expected_hot={'hot2-a':'2','hot2-direct-maintained':'2','hot2-direct-maintained-64k':'2'}[variant]",
    ),
    (
        ...
        "expected_map={'hot2-a':'0','hot2-direct-maintained':'0','hot2-direct-maintained-64k':'0'}[variant]",
    ),
    (
        ...
        "expected_snapshot={'hot2-a':'2000','hot2-direct-maintained':'2000','hot2-direct-maintained-64k':'65536'}[variant]",
    ),
    (
    ...
    (
        ...
        "maintenance_enabled=variant in ('hot2-direct-maintained','hot2-direct-maintained-64k')",
    ),
):
    runner_text = replace_once(runner_text, old, new, "P11 runner")
...
for variant, settings in variants.items():
    ...
    old_mount = f"{target_exp_text}/variants/hot2-a/flink-conf.yaml"
    ...
    for key, value in (
        ("CACHEKIT_MAP_DIRTY_OVERLAY_ENABLED", settings["overlay"]),
        (
            "CACHEKIT_MAP_SNAPSHOT_MAINTENANCE_ENABLED",
            settings["maintenance"],
        ),
    ):
        pattern = re.compile(rf"(?m)^(\s*){key}: '(?:true|false)'$")
        matches = list(pattern.finditer(text))
        if len(matches) != 4:
            raise SystemExit(
                f"runtime gate count for {key} is {len(matches)} "
                f"in {compose}; expected 4"
            )
        text = pattern.sub(
            lambda match: match.group(1) + key + ": '" + value + "'",
            text,
        )
    ...
    if indexed_key not in text:
        text += (
            "\n# Same-artifact P11 control; indexed write batch remains disabled.\n"
            f"{indexed_key}: false\n"
        )
...
identity.update(
    {
        "schema": "cachekit-direct-snapshot-maintenance-p11-kunpeng-screen-v1",
        "phase": (
            "kunpeng-numa0-q9-same-artifact-"
            "direct-incremental-exact-snapshot-screen"
        ),
        ...
        "a_control": "hot2-a",
        "mechanism_treatment": "hot2-direct-maintained",
        "treatment": "hot2-direct-maintained-64k",
        ...
        "claim_boundary": (
            "same Kunpeng NUMA0 physical cluster and P11 artifact; the "
            "coexistence preflight proves the selected CPUs idle and records "
            "any foreign containers; dirty overlay is disabled and direct "
            "exact-snapshot maintenance activation is explicit"
        ),
    }
)
identity["artifact_build"] = {
    "p10_aarch64_base_sha256": (
...
config_differences = {}
for left, right in (
    ("hot2-a", "hot2-direct-maintained"),
    ("hot2-direct-maintained", "hot2-direct-maintained-64k"),
):
    values = {
...
expected_config = {
    "hot2-a_vs_hot2-direct-maintained": set(),
    "hot2-direct-maintained_vs_hot2-direct-maintained-64k": {
        "state.backend.cachekit.map.snapshot.cache.max-entries"
    },
}
compose_env = {}
...
audit = {
    "schema": "cachekit-p11-config-diff-v1",
...
preflight = {
    "schema": "cachekit-p11-kunpeng-numa-coexistence-preflight-v1",
    "target_node": 0,
...
staging_audit = {
    "schema": "cachekit-p11-kunpeng-staging-audit-v1",
    "valid": True,
...
def main():
    ...
    candidate = (
        ...
        "flink-statebackend-cachekit-1.16-SNAPSHOT-p11-aarch64.jar"
    )
    ...
    with tempfile.TemporaryDirectory(prefix="cachekit-p11-kunpeng-stage-") as tmp:

...
expected_source=56aaafe595fa51df07a4ea371f255751375d7884
...
./mvnw -pl flink-state-backends/flink-statebackend-cachekit \
  "${common_maven[@]}" -Dtest=CachedInternalMapStateTest test

rocks_classes=$repo/flink-state-backends/flink-statebackend-rocksdb/target/classes
cachekit_classes=$repo/flink-state-backends/flink-statebackend-cachekit/target/classes
target_dir=$repo/flink-state-backends/flink-statebackend-cachekit/target
overlay=$(mktemp -d /tmp/cachekit-p9-overlay.XXXXXX)
cleanup() {
  [[ $overlay == /tmp/cachekit-p9-overlay.* && -d $overlay ]] && rm -rf -- "$overlay"
}
trap cleanup EXIT

copy_stem() {
  local classes=$1 stem=$2 found=0 class_file relative
...
for stem in "${rocks_stems[@]}"; do
  copy_stem "$rocks_classes" "$stem"
done
copy_stem "$cachekit_classes" \
...
build_one() {
  ...
  candidate=$target_dir/flink-statebackend-cachekit-1.16-SNAPSHOT-p9-$arch.jar

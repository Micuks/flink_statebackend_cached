...
expected_source=a121bb6229fa8538335ac23ebacec50d2e44c912
expected_x86_p17_sha=bb9d7755dd5c71093a9739badeac1116a447a9e7b6f04365aca7c1f110b4b27e
expected_arm_p17_sha=1fb610849d2e518aa34e132745909b7ce13b8792b985978215a9ef7822342969
...
overlay=$(mktemp -d /tmp/cachekit-p18-overlay.XXXXXX)
cleanup() {
  [[ $overlay == /tmp/cachekit-p18-overlay.* && -d $overlay ]] && rm -rf -- "$overlay"
}
trap cleanup EXIT
...
build_one() {
  ...
  base=$target_dir/flink-statebackend-cachekit-1.16-SNAPSHOT-p17-$arch.jar
  candidate=$target_dir/flink-statebackend-cachekit-1.16-SNAPSHOT-p18-$arch.jar
  [[ -f $base ]] || { echo "missing P17 base: $base" >&2; exit 68; }
  base_sha=$(sha256sum "$base" | awk '{print $1}')
  [[ $base_sha == "$expected_base_sha" ]] || {
    echo "unexpected $arch P17 base: $base_sha" >&2
    exit 69
  }
  cp "$base" "$candidate"
  ...
  diff -u "$overlay/$arch.base.entries" "$overlay/$arch.candidate.entries" || {
    echo "$arch candidate entry set differs from P17" >&2
    exit 70
  }
...
echo "schema=cachekit-p18-artifact-overlay-audit-v1"
echo "source_commit=$expected_source"
echo "focused_tests=43"
echo "overlay_stem=$stem"
echo "entry_set_identical_to_p17=true"
echo "non_overlay_entries_identical_to_p17=true"
build_one x86 "$expected_x86_p17_sha"
build_one aarch64 "$expected_arm_p17_sha"

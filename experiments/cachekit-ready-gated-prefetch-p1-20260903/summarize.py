#!/usr/bin/env python3
"""Summarize the CacheKit P1 q9 canary and its advancement gate."""
...
def main() -> None:
    ...
    parser.add_argument("--output-prefix", default="P1_RESULT")
    ...
    for result_path in sorted((args.expdir / "results" / "raw").glob("*/result.json")):
        ...
        activation_path = leg / "READY_GATE_AUDIT.json"
        activation = (
    ...
    result = {
        "schema": "cachekit-ready-gated-prefetch-p1-summary-v1",
    ...
    result["q9_gate"] = {
        ...
        "activation_pass": bool(candidate_rows)
        and all(row["activation"] and row["activation"]["activation_pass"] for row in candidate_rows),
        "performance_pass": bool(ranking)
        and ranking[0]["arithmetic_mean_uplift_pct"] >= 3.0,
    }
    result["q9_gate"]["pass"] = (
        result["q9_gate"]["activation_pass"]
        and result["q9_gate"]["performance_pass"]
    )

    lines = [
        "# CacheKit ready-gated prefetch P1 q9 result",
        "",
    ...
    for row in rows:
        lines.append(
            ...
            f"{'yes' if row['activation'] and row['activation']['activation_pass'] else 'no'} |"
        )
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            ...
            "## q9 advancement gate",
            "",
            f"- Activation: {'PASS' if result['q9_gate']['activation_pass'] else 'FAIL'}",
            f"- K/s/core uplift >= +3.00%: {'PASS' if result['q9_gate']['performance_pass'] else 'FAIL'}",
            f"- Decision: {'ADVANCE' if result['q9_gate']['pass'] else 'REJECT'}",
        ]
    )
    markdown = "\n".join(lines) + "\n"

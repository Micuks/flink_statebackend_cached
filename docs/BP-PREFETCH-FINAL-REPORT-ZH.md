...
日期:2026-07-08(v3,含第二轮优化)
代码:`flink-statebackend-cached-bp-prefetch` @ `20bb6dc265`(base:feat/cachekit-map-snapshot-cache)
...
| 指标 | 数值 |
|---|---|
| 15q throughput/core 均值(vs 纯 RocksDB,50M×3 轮交错,坏样本清洗后,深夜安静时段) | **+32.43%** |
| 剔除 q12(proc-time 非确定 query)后 | **+34.72%** |
...
|---|---:|---:|---:|---|
| q3 | 439.7 | 446.7 | +1.6% | 早期轮次的 ±7% 摆动经夜测收敛为噪声带,≈持平 |
| q4 | 82.1 | 101.6 | **+23.7%** | |
| q5 | 199.7 | 182.9 | -8.4% | 滑动窗口;唯一遗留真回退(深夜 3 轮定论) |
| q7 | 91.5 | 92.3 | +0.9% | |
| q8 | 411.2 | 446.0 | **+8.5%** | VoidNamespace 门控 + 预取空转短路两级修复(最初 -10.2%) |
| q9 | 57.9 | 57.5 | -0.8% | |
| q11 | 125.5 | 128.1 | +2.1% | 门控放弃窗口缓存收益(门控前 +12.9%,正确性优先) |
| q12 | 304.5 | 305.6 | +0.4% | proc-time query,历史口径剔除 |
| q13 | 131.0 | 126.4 | -3.6% | ±4% 摆动带 |
| q15 | 41.0 | 135.6 | **+318%** | wall 740s→121s、总 TPS ~12×,收益真实;rocksdb 腿 cores 采样偏低使 per-core 口径需注明 |
| q16 | 20.2 | 32.2 | **+59.6%** | wall 421s→273s |
| q17 | 214.3 | 334.4 | **+56.1%** | |
| q18 | 148.3 | 192.9 | **+30.0%** | dedup fold 后(fold 前 +24.2%) |
| q19 | 143.0 | 142.6 | -0.2% | |
| q20 | 78.2 | 77.0 | -1.5% | |

清洗规则:剔除 cores≤1 或 >16 的采样轮(q15 rocksdb 有一轮 cores=1);q12 按 proc-time 口径单列。数据为 2026-07-07 22:00 深夜安静时段 3 轮(final3r-v3)。
...
4. **预取空转短路**(`26106b250a`,第二轮):backend 暴露 `hasPrefetchableState()`,StatePrefetcher 在付出每 batch 的 key 提取代价前先探测(wrapper lazy 注册,须每 batch 重查);窗口算子在 VoidNamespace 门控下无可预取 state,此前每个 lookahead batch 白做最多 distance 个 key 的提取。q8 +2.9%→+8.5%,q3 转入噪声带。
5. **dedup 本地折叠**(`20bb6dc265`,第二轮):`RowTimeDeduplicateFunction` 实现 `BatchableKeyedFunction`——同 key batch 按 `isDuplicate` 链扫描折叠出 winner,keyed state 一读一写、emit 单一 transition;被跳过的中间 emission 是 UPDATE_BEFORE/UPDATE_AFTER 冗余对(CDC 终态不变,与 GroupAgg fold 同一论证)。q18 +24.2%→+30.0%,CDC(排序源)SUCCESS。
...
- 最终 perf:`results-cachekit-bp-prefetch/final3r-v3_20260707_220000/`(90/90 legs,深夜安静时段);历史:final3r-v2_20260707_002010、shortcircuit_20260707_112627、foldq18_20260707_135740

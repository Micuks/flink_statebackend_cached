...
public class RocksDBStateBackendConfigTest {
    ...
    @Test
    public void testArmPointMapFlatAuthoritySemantics() throws Exception {
        Assume.assumeTrue(
        ...
        configuration.set(
                RocksDBConfigurableOptions.MEMTABLE_ARM_POINT_MAP_FLAT_AUTHORITY, true);
        ...
        try {
            final MapStateDescriptor<Integer, Integer> descriptor =
                    new MapStateDescriptor<>("arm-point-map-flat", Integer.class, Integer.class);

            keyedBackend.setCurrentKey(1);
            final MapState<Integer, Integer> firstNamespace =
                    keyedBackend.getPartitionedState(
                            "namespace-a", StringSerializer.INSTANCE, descriptor);
            for (int i = 0; i < 64; i++) {
                firstNamespace.put(i, i * 10);
            }
            firstNamespace.put(7, 7000);
            firstNamespace.remove(8);

            final Map<Integer, Integer> observed = new HashMap<>();
            for (Map.Entry<Integer, Integer> entry : firstNamespace.entries()) {
                observed.put(entry.getKey(), entry.getValue());
            }
            assertEquals(63, observed.size());
            assertEquals(Integer.valueOf(7000), observed.get(7));
            assertFalse(observed.containsKey(8));
            assertEquals(Integer.valueOf(90), firstNamespace.get(9));
            assertTrue(firstNamespace.contains(9));
            ...
            assertEquals(
                    ...
                    keyedBackend
                            .getPartitionedState(
                                    "namespace-a", StringSerializer.INSTANCE, descriptor)
                            .get(7));
            secondNamespace.clear();
            assertFalse(secondNamespace.entries().iterator().hasNext());
        } finally {

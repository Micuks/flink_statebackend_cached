...
import org.apache.flink.contrib.streaming.state.cacheplus.NoOpCachePolicy;
...
public class CachingInternalMapState<K, N, UK, UV>
    ...
    private static final ThreadLocal<Boolean> THREAD_LOCAL_BYPASS = ThreadLocal.withInitial(() ->
        Boolean.FALSE
    );
    private static final ThreadLocal<Boolean> THREAD_LOCAL_NO_TOUCH = ThreadLocal.withInitial(() ->
        Boolean.FALSE
    );
    ...
    private static JoinSide detectJoinSideFromStack() {
        ...
        try {
            ...
            for (int i = 2; i < maxDepth; i++) {
                ...
                if (
                    "processElement1".equals(method) ||
                    method.contains("processElement1") ||
                    "processRecord1".equals(method) ||
                    method.contains("processRecord1")
                ) {
                    return JoinSide.LEFT;
                }
                if (
                    "processElement2".equals(method) ||
                    method.contains("processElement2") ||
                    "processRecord2".equals(method) ||
                    method.contains("processRecord2")
                ) {
        ...
        if (!LOGGED_UNKNOWN_ONCE) {
            ...
            LOG.debug(
                "Join side could not be detected from stack; using conservative mode (no auto-bypass)."
            );
        }
    ...
    private class PerKeyMapCache<UK_C, UV_C, K_F, N_F> {
        ...
        private final CachingPlusKeyedStateBackend<K_F> ownerBackend;
        ...
        PerKeyMapCache(
            ...
            this.userKeySerializerView = ThreadLocal.withInitial(() -> {
            ...
            this.userKeyDeserializerView = ThreadLocal.withInitial(() -> {
                if (this.userKeySerializer != null) {
            ...
            this.userValueSerializerView = ThreadLocal.withInitial(() -> {
            ...
            this.userValueDeserializerView = ThreadLocal.withInitial(() -> {
                if (this.userValueSerializer != null) {
                    return new DataInputDeserializer();
                } else {
                    return new DataInputDeserializer(new byte[0]);
            ...
            if (
                ...
                l2Size > 0 &&
                ownerBackend.getManagedPagePool() != null &&
                ownerBackend.getManagedPagePool().isUsable()
            ) {
                this.l2MapEntriesOffHeap = new OffHeapKVStore(
                    ownerBackend.getManagedPagePool(),
                    ownerBackend,
                    ownerBackend.getL2TimeBucketSizeMillis()
                );
                this.l2MapEntries = new NoOpCachePolicy<>();
            } else {
                if (l2ManagedMemoryEnabled && l2Size > 0) {
                    LOG.info(
                        "Falling back to on-heap L2 cache for this PerKeyMapCache because managed memory is not available."
                    );
                }
                this.l2MapEntriesOffHeap = null;
                this.l2MapEntries = createCachePolicyInstance(
                    cachePolicyType,
                    l2Size,
                    null,
                    ownerBackend,
            ...
            this.l1MapEntries = createCachePolicyInstance(
                cachePolicyType,
                l1Size,
                evictedL1MapEntry -> {
                    ...
                    try {
                        ...
                        originalDelegateNamespaceContext =
                            (N_F) CachingInternalMapState.this.getCurrentNamespace();
                        LOG.debug(
                            "L1-Evict-Flush: CONTEXT SWITCH. Original(key={}, ns={}). Setting to(key={}, ns={}) for UK {}",
                            originalKeyContext,
                            originalDelegateNamespaceContext,
                            flinkKey,
                            cacheNamespace,
                            evictedL1MapEntry.getKey()
                        );
                        ...
                        if (evictedUVWrapper.getValue() == null) {
                            ...
                            try {
                                if (l2ManagedMemoryEnabled && l2MapEntriesOffHeap != null) {
                                    ...
                                    try {
                                        ...
                                        if (
                                            ...
                                            LOG.info(
                                                "L2 off-heap cache is full. Attempting to evict to make space for key '{}'.",
                                                evictedUK
                                            );
                                            long spaceNeeded =
                                                serializedKey.length + serializedValue.length + 128; // 128 bytes buffer
                                            long freed = this.l2MapEntriesOffHeap.evict(
                                                spaceNeeded
                                            );
                                            if (freed > 0) {
                                                try {
                                                    ...
                                                    LOG.warn(
                                                        "Failed to add entry for key '{}' to L2 cache even after evicting {} bytes. This may impact performance.",
                                                        evictedUK,
                                                        freed,
                                                        e2
                                                    );
                                                ...
                                                LOG.warn(
                                                    "Could not evict any data from L2 cache for key '{}'. Entry will not be cached in L2. This may impact performance.",
                                                    evictedUK
                                                );
                                            }
                                        } else {
                                ...
                                throw new RuntimeException(
                                    "Failed to write to L2 cache during L1 eviction",
                                    e
                                );
                            }
                        }
                        ...
                        throw new RuntimeException(
                            "Failed to flush L1 map entry to delegate for user key: " + evictedUK,
                            e
                        );
                    } finally {
                        ...
                        LOG.debug(
                            "L1-Evict-Flush: CONTEXT RESTORE. Before restore(key={}, ns={}). Restored to(key={}, ns={}). For UK {}",
                            keyAfter,
                            cacheNamespace,
                            originalKeyContext,
                            originalDelegateNamespaceContext,
        ...
        private <CK, CV_ENTRY_TYPE> CachePolicy<
            CK,
        ...
        private long fingerprint(UK_C userKey) {
            try {
                ...
                throw new RuntimeException(
                    "Error serializing user key for presence cache fingerprint",
                    e
                );
            }
        }
        ...
        ValuePresence getValuePresence(UK_C userKey) {
            ...
            if (
                ...
                if (l1Presence != null) {
                    return l1Presence == PrimitivePresenceCache.PRESENT
                        ? ValuePresence.PRESENT_IN_CACHE_CLEAN
                ...
                if (l2Presence != null) {
                    ...
                    if (oldL1 == null) ownerBackend.reportCacheMemoryAdded(16L); // New entry in L1
                    return l2Presence == PrimitivePresenceCache.PRESENT
                        ? ValuePresence.PRESENT_IN_CACHE_CLEAN
                        : ValuePresence.ABSENT_IN_CACHE;
                }
                return ValuePresence.ABSENT_MAYBE_IN_VALUE_CACHE;
                ...
                if (l2Presence != null) {
                    ...
                    if (oldL1 == null) ownerBackend.reportCacheMemoryAdded(
                        ValueSizeUtils.estimate(Boolean.TRUE)
                    );
        ...
        void updatePresenceCacheOnGet(UK_C userKey, boolean valuePresentInValueCacheOrDelegate) {
            ...
            if (
                ...
                CacheEntry<Boolean> oldL1P = l1KeyPresenceCache.put(
                    userKey,
        ...
        void invalidatePresenceCache(UK_C userKey) {
            ...
            if (
                ...
                if (oldL2P != null) ownerBackend.reportCacheMemoryReleased(
                    ValueSizeUtils.estimate(Boolean.TRUE)
                );
            }
        }
        ...
        void registerMetrics(MetricGroup group, K_F key, N_F namespace) {
            ...
            if (l2ManagedMemoryEnabled && l2MapEntriesOffHeap != null) {
                ...
                metricGroup.gauge("l2OffHeapBytes", () ->
                    l2MapEntriesOffHeap.getEstimatedMemoryUsageBytes()
                );
                metricGroup.gauge("l2OffHeapEntries", () -> l2MapEntriesOffHeap.size());
                metricGroup.gauge("l2OffHeapPagesFreedCapacity", () ->
                    l2MapEntriesOffHeap.getPagesFreedByCapacity()
                );
                metricGroup.gauge("l2OffHeapPagesFreedWatermark", () ->
                    l2MapEntriesOffHeap.getPagesFreedByWatermark()
                );
                metricGroup.gauge("l2OffHeapPagesFreedPerSec", () ->
                    l2MapEntriesOffHeap.getApproxPagesFreedPerSecond()
                );
            }
            ...
            if (keyPresenceCacheEnabled) {
                if (
                    ...
                    totalSize +=
                        ...
                        16L; // Approx. size per entry
                } else {
                    totalSize +=
                        (long) (l1KeyPresenceCache.size() + l2KeyPresenceCache.size()) *
                        ValueSizeUtils.estimate(Boolean.TRUE);
                }
            }
            return totalSize;
        ...
        long evictToMeetMemoryLimit(long bytesToFree) {
            ...
            Iterator<Map.Entry<UK_C, CacheEntry<UV_C>>> l1Iterator = l1MapEntries
                .entrySet()
                .iterator();
            ...
            for (Map.Entry<Long, Byte> entry : keysToRemove) {
                ...
                if (cache.remove(key) != null) {
                    if (cache == l1PrimitivePresenceCache) {
                        ...
                        // It is the l2PrimitivePresenceCache
        ...
        private <ENTRY_KEY, ENTRY_VAL> long evictFromCache(
            ...
            long actualFreed = 0;
            Iterator<Map.Entry<ENTRY_KEY, CacheEntry<ENTRY_VAL>>> iterator = cache
                .entrySet()
                .iterator();
            ...
            while (iterator.hasNext() && actualFreed < requiredBytes) {
                ...
                if (isPresence) {
                    ...
                    if (!cacheEntry.isDirty()) {
                        // Only evict clean entries from L2 value cache
                        // this way
                ...
                if (flinkKeyCaches != null) {
                    try {
                        ...
                        if (NCDK != null) {
                            ...
                            // It is safe to set null here: CachingPlusKeyedStateBackend#setCurrentKey(null)
    ...
    private PerKeyMapCache<UK, UV, K, N> getOrCreatePerKeyMapCache() {
        ...
        CachePolicy<K, PerKeyMapCache<UK, UV, K, N>> flinkKeyCaches =
            namespaceCaches.computeIfAbsent(namespace, n -> {
                    ...
                    evictedFlinkKeyEntry -> {
                        ...
                        if (perKeyCache != null) {
                            try {
                                ...
                                if (originalKey != null) backend.setCurrentKey(originalKey);
                                else backend.setCurrentKey(null);
                                if (originalNamespace != null) delegateState.setCurrentNamespace(
                                    originalNamespace
                                );
                                else delegateState.setCurrentNamespace(null);
                            } catch (Exception e) {
        ...
        return flinkKeyCaches.computeIfAbsent(key, k -> {
            PerKeyMapCache<UK, UV, K, N> perKeyCache = new PerKeyMapCache<>(
            ...
            if (
                CachingInternalMapState.this.metrics != null &&
                CachingInternalMapState.this.perKeyMetricsEnabled
            ) {
                perKeyCache.registerMetrics(
                    CachingInternalMapState.this.metrics.addGroup("perKeyCache"),
                    k,
                    getCurrentNamespace()
                );
            }
    ...
    private void registerProfileMetricsIfNeeded() {
        ...
        try {
            ...
            profContainsCalls = pg.counter("contains_calls");
            ...
            pg.gauge("contains_totalNanos", () -> profContainsTotalNanos.get());
            ...
            pg.gauge("contains_avgMicros", () -> {
                long c = profContainsCalls.getCount();
                return c > 0 ? (profContainsTotalNanos.get() / (double) c) / 1000.0 : 0.0;
            });
            LOG.info("Successfully registered profile metrics for MapState.");
        } catch (Exception e) {
            LOG.error("Failed to register profile metrics for MapState", e);
        }
    }
    ...
    private long sumL2Entries() {
        ...
        for (CachePolicy<K, PerKeyMapCache<UK, UV, K, N>> m : namespaceCaches.values()) {
            ...
            for (Map.Entry<K, PerKeyMapCache<UK, UV, K, N>> e : m.entrySet()) {
                PerKeyMapCache<UK, UV, K, N> pc = e.getValue();
                if (pc == null) continue;
                if (pc.l2ManagedMemoryEnabled && pc.l2MapEntriesOffHeap != null) s +=
    ...
    private void updateCacheBypassConditionWeighted(boolean resolvedByCache, int weight) {
        ...
        if (
            !bypassEnabled ||
            this.mapCacheHitRateThreshold <= 0.0 ||
    ...
    public UV get(UK userKey) throws Exception {
        ...
        try {
            ...
            if (!this.keyPresenceCacheEnabled) {
                ...
                if (l1Entry != null) {
                    ...
                    try {
                        ...
                        if (
                            ...
                            byte[] serializedL2Value = perKeyCache.l2MapEntriesOffHeap.get(
                                serializedKey
                            );
                        ...
                        if (l2Entry != null || userValue != null) {
                            ...
                            CacheEntry<UV> oldL1Entry = perKeyCache.l1MapEntries.put(
                                userKey,
                                newL1Entry
                            );
                            if (oldL1Entry != null) {
                            ...
                            if (userValue != null) {
                                ...
                                CacheEntry<UV> oldEntry = perKeyCache.l1MapEntries.put(
                                    userKey,
                                    newEntry
                                );
                                if (oldEntry != null) {
                ...
                updateCacheBypassConditionWeighted(
                    resolvedByCache && userValue != null,
                    decisionWeight
                );
                return userValue;
            }
            ...
            if (l1ValEntry != null) {
                ...
                if (
                    presence == ValuePresence.ABSENT_MAYBE_IN_VALUE_CACHE
                ) l1PresenceCacheMissCount.inc();
                // Count initial presence miss
    ...
    public void put(UK userKey, UV userValue) throws Exception {
        // ===================== START: CUSTOM LOGGING FOR VERIFICATION =====================
        // Simple sampling to avoid log flooding. Log roughly 1 in 10000 calls.
        if (Math.random() < 0.0001) {
            try {
                // Get the current key that Flink is processing (e.g., auction.id or category)
                Object flinkKey = backend.getCurrentKey();
                // Get the namespace (often used for windowing)
                Object namespace = getCurrentNamespace();

                // It's good practice to check for nulls
                String flinkKeyStr = (flinkKey == null) ? "null" : flinkKey.toString();
                String namespaceStr = (namespace == null) ? "null" : namespace.toString();
    ...
    private void loadAllEntriesToCache(PerKeyMapCache<UK, UV, K, N> perKeyCache) throws Exception {
        ...
        if (entriesFromDelegate != null) {
            for (Map.Entry<UK, UV> entry : entriesFromDelegate) {
                if (entry.getValue() != null) {
                    // Don't cache null values from delegate in value
                    // cache
                    CacheEntry<UV> newCacheEntry = CacheEntry.clean(entry.getValue());
                    CacheEntry<UV> oldCacheEntry = perKeyCache.l1MapEntries.put(
                        entry.getKey(),
                        newCacheEntry
                    );
                    ...
                    // Null value from delegate for a key means it effectively doesn't exist
                    // for value cache.
                    if (this.keyPresenceCacheEnabled) {
    ...
    public void onWatermarkEvict(long watermarkMillis) {
        try {
            ...
            > nsEntry : namespaceCaches.entrySet()) {
                ...
                List<PerKeyMapCache<UK, UV, K, N>> caches = new ArrayList<>();
    ...
    public long evictEntriesToFreeMemory(long targetBytesToFreeThisState) {
        ...
        try {
            ...
            LOG.warn(
                "Error during explicit memory eviction from CachingInternalMapState: {}",
                e.getMessage(),
                e
            );
        }
        // The actual amount freed is tracked by CachingPlusKeyedStateBackend's atomic counter.
    ...
    private class UnionIterator implements Iterator<Map.Entry<UK, UV>>, AutoCloseable {
        ...
        UnionIterator(
            ...
            this.l2Iterator = usingOffHeapL2
        ...
        private boolean emitFromL2OnHeap() {
            Iterator<Map.Entry<UK, CacheEntry<UV>>> iterator = (Iterator<
                Map.Entry<UK, CacheEntry<UV>>
            >) l2Iterator;
    ...
    private <K_F, N_F, UK_C, UV_C> void flushL1Entries(
        PerKeyMapCache<UK_C, UV_C, K_F, N_F> perKeyCache,
        K_F flinkKey,
        N_F namespace,
        CachingPlusKeyedStateBackend<K_F> backendForContext,
        InternalMapState<K_F, N_F, UK_C, UV_C> delegateStateForContext,
        N_F originalNamespaceToRestore
    ) throws Exception {
        if (perKeyCache.l1MapEntries.isEmpty()) {
            return;
        }
            ...
            for (Map.Entry<UK_C, CacheEntry<UV_C>> l1Entry : perKeyCache.l1MapEntries.entrySet()) {
                ...
                if (cacheEntry.isDirty()) {
                        ...
                        if (
                            ...
                            try {
                                ...
                                if (
                                    ...
                                    long freed = perKeyCache.l2MapEntriesOffHeap.evict(
                                        serializedKey.length + serializedValue.length + 128L
                                    );
                                    if (freed > 0) {
                                        try {
                                            perKeyCache.l2MapEntriesOffHeap.put(
                                                serializedKey,
                                                serializedValue
                                            );
                                        } catch (IOException retry) {
                                            LOG.warn(
                                                "L2 off-heap put still failed after evicting {} bytes.",
                                                freed,
                                                retry
                                            );
                                        }
                                        ...
                                        LOG.warn(
                                            "L2 off-heap eviction freed 0 bytes; skipping L2 insert."
                                        );
                                    }
                                } else {
                                    throw ioe;
                    ...
                    if (
                        ...
                        perKeyCache.l2MapEntries.put(
                            userKey,
                            CacheEntry.clean(cacheEntry.getValue())
                        );
                    }
                }
            }
    ...
    private void logCacheMetrics() {
        if (LOG.isDebugEnabled()) {
            // Calculate hit rates
            double l1ValueHitRate = calculateHitRate(
                l1MapValueCacheHitCount,
                l1MapValueCacheMissCount
            );
            double l2ValueHitRate = calculateHitRate(
                l2MapValueCacheHitCount,
                l2MapValueCacheMissCount
            );
            double l1PresenceHitRate = calculateHitRate(
                l1PresenceCacheHitCount,
                l1PresenceCacheMissCount
            );
            double l2PresenceHitRate = calculateHitRate(
                l2PresenceCacheHitCount,
                l2PresenceCacheMissCount
            );
            ...
            LOG.debug(
                ...
                l1ValueHitRate,
                l2MapValueCacheHitCount.getCount(),

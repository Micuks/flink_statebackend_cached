...
public class CacheKitStateBackendFactory implements StateBackendFactory<CacheKitStateBackend> {
    ...
    public static final ConfigOption<Boolean> MAP_SNAPSHOT_CACHE_NATIVE_ENABLED =
            ConfigOptions.key("state.backend.cachekit.map.snapshot.cache.native.enabled")
                    .booleanType()
                    .defaultValue(false)
                    .withDescription(
                            "Store MapState EMPTY/SINGLE snapshots in the standalone JNI cache. "
                                    + "The Java snapshot cache remains the default.");

    public static final ConfigOption<Boolean> MAP_SNAPSHOT_CACHE_NATIVE_CLASSIFIER_ENABLED =
            ConfigOptions.key("state.backend.cachekit.map.snapshot.cache.native.classifier.enabled")
                    .booleanType()
                    .defaultValue(false)
                    .withDescription(
                            "Reserved compatibility option for the withdrawn native RocksDB "
                                    + "prefix classifier. Must remain false.");

    public static final ConfigOption<Boolean> MAP_SNAPSHOT_CACHE_NATIVE_REMOVE_HINT_ENABLED =
            ConfigOptions.key(
                            "state.backend.cachekit.map.snapshot.cache.native.remove-hint.enabled")
                    .booleanType()
                    .defaultValue(false)
                    .withDescription(
                            "Skip JNI removes when an exact Java-side hash-count hint proves absence.");

    public static final ConfigOption<String> MAP_SNAPSHOT_CACHE_NATIVE_LIBRARY_PATH =
            ConfigOptions.key("state.backend.cachekit.map.snapshot.cache.native.library-path")
                    .stringType()
                    .defaultValue("")
                    .withDescription(
                            "Absolute libcachekit_snapshot_jni.so path. Empty loads the library "
                                    + "embedded in the CacheKit JAR, with java.library.path fallback.");
	...
	public CacheKitStateBackend createFromConfig(ReadableConfig config, ClassLoader classLoader)
		...
		final NativeMapSnapshotOptions nativeMapSnapshotOptions =
				nativeMapSnapshotOptions(config);
	...
	static NativeMapSnapshotOptions nativeMapSnapshotOptions(ReadableConfig config) {
		return new NativeMapSnapshotOptions(
